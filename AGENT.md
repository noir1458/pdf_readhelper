...
### 2026-09-12 — Treat a spread as one reading view with two page translations

**Decision:** In spread layout, show the cover-first visible page pair comma-separated in the toolbar field and treat that pair as the AI panel's active view. Manual translation submits both page images independently, while AUTO submits only missing members after its settle delay. Render page-specific provenance and text in left-to-right order with a divider, and keep independent request cancellation, cache, error, and retry state per page.
